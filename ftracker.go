...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	speed := meanSpeed(action, duration)
	TotalCaloriesRunning := (runningCaloriesMeanSpeedMultiplier * speed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
	return TotalCaloriesRunning
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	speedMeterPerSec := meanSpeed(action, duration) * kmhInMsec
	return (walkingCaloriesWeightMultiplier * weight + (math.Pow(speedMeterPerSec, 2)/(height*0.01))*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
